...
class DiagnosticController extends Controller
{
    ...
    public function lastHeartbeatExport(Request $request){
        ...
        try {
            ...
            if($request->date != '')
            {
                $app->where('tms_v_last_heart_beat.update_time', 'ilike', '%'.$request->date.'%');
            }

            if($request->terminalIds != '')
            {
                $app->join('tms_terminal','tms_terminal.sn','=','tms_v_last_heart_beat.sn')
                ->whereIn('tms_terminal.id', $request->terminalIds);
            }

            if($request->terminalGroupsIds != '')
            {
                $app->join('tms_terminal','tms_terminal.sn','=','tms_v_last_heart_beat.sn')
                ->join('tms_terminal_group_link','tms_terminal_group_link.terminal_id','=','tms_terminal.id')
                ->whereIn('tms_terminal_group_link.terminal_group_id', $request->terminalGroupsIds);
            }
    ...
    public function lastDiagnosticExport(Request $request){
        // $validator = Validator::make($request->all(), [
        //     'sn' => 'required_without:terminalId|max:30',
        //     'terminalId' => 'required_without:sn|max:36'
        ...
        try {
            ...
            select(
                ...
                'update_time as updateTime',
                'installed_apps_string as installedAppsString'
                //'tms_terminal.id as TID'
                
            )
            //->join('tms_terminal','tms_terminal.sn','=','tms_v_last_diagnostic_info.sn')
            ->where('tms_v_last_diagnostic_info.tenant_id',$request->header('Tenant-id'));
            //->where('tms_v_last_diagnostic_info.tenant_id',"testing");
            ...
            if($request->date != '')
            {
                $app->where('tms_v_last_diagnostic_info.update_time', 'ilike', '%'.$request->date.'%');
            }
